...
void eval(char *cmdline)
{
    ...
    char buf[MAXLINE];   /* Holds modified command lin */
    int bg;              /* Should the job run in bg or fg? */
	pid_t pid;           /* Process id */
	int cd_check;		/*cd이면 exe안돌리려고*/  
	...
	if (!builtin_command(argv)) { //quit -> exit(0), & -> ignore, other -> run
		...
		if((cd_check=strcmp(argv[0],"cd"))==0){
		if(chdir(argv[1])!=0)
		perror("cd");
	}
       if((pid = Fork()) == 0) { // child runs
